...
data["Height (feet)"] = data["Height (feet)"].str.replace(r'\s', '', regex=True)
#Clean Showy
def is_showy(x):
    if isinstance(x,float):
        return x
...
def clean_bloom(cell_value):
    if cell_value in [" Non-flowering", " Rarely flowers"]:
        return ""
    if "frost" in cell_value:
        return ""
    x = cell_value.split(" to ")
...
def clean_exposure(cell_value):
    ...
    for i,val in enumerate(x):
        if val.lower().strip() == "full sun":
            x[i] = "Sun"
            continue
        if val.lower() == "full shade":
            x[i] = "Shade"
            continue
        if val.lower() == "part shade":
            x[i] = "Part Shade"
...
def determine_height(cell_value):
    ...
    if measurement.lower() in ["in","inches"]:
        ...
        return cell_value.replace(measurement,"").strip().replace("-","–")
...
data["Soil Moisture"] = data["Soil Moisture"].str.strip().str.replace(",",", ")
