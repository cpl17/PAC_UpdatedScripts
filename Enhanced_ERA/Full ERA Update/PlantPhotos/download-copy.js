...
async function downloadMain() {
  ...
  //The plants in in central google sheets
  ...
  //remove records where _id is not in an array of scientific names
  await plants.remove({
  ...
  for (const record of records) {
    ...
    //Check if plant already in plants collection
...
//Write nurseries to the nurseries collection in mongo
...
//Write online stores from ONLINE google sheets to mongoDB
    ...
    await plants.updateOne({
      ...
      $push: {
        'Online Stores': {
          ...
          label: record.Root.trim() //ErnstSeed.com
        }
      }
    });
  }
...
//Check wikimedia common search for image
...
//Resize and store the image locally in /images
...
//Write the clean object to the mongoDB
...
//Use node.fetch to make a http request for google sheets and wiki urls
